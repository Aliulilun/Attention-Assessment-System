...
class InteractionEngine:
    ...
    def __init__(
        ...
        # ============================================================
        # 🌟 新增（B2）：跳幀補償
        # dwell_counters 是「analyze_interaction 被呼叫幾次」，不是影片幀數。
        # 主迴圈只在 frame_count % YOLO_SKIP == 0 時呼叫本函式，所以
        # DWELL_FRAMES=3 實際代表 3 × YOLO_SKIP 幀、S8_MIN_DWELL_FRAMES=10
        # 實際代表 10 × YOLO_SKIP 幀（註解寫的「≈0.33s@30fps」只在
        # YOLO_SKIP=1 時成立）。UI 允許使用者自由調整跳幀，一旦調成 2，
        # 同一支影片就會跑出不同結果。
        # 呼叫端請用 set_frame_stride() 告知目前的 YOLO_SKIP，
        # 門檻會自動換算回「真實幀數」語意。預設 1，行為與原本完全相同。
        # ============================================================
        ...
        # ============================================================
        # 🌟 新增（A1）：施測者頭部框
        # TH（看回施測者）原本用「畫面左 35% 的滿版長條」當判定框。
        # 兒童眼睛在右側，只要視線 x 分量為負就必定與該框相交——
        # 框測試在數學上恆真，真正生效的只有 pitch/yaw 兩個角度門檻。
        # 這裡輸出施測者真實的頭部框，讓主迴圈拿去做 TH 判定。
        # 每幀更新；取不到時（施測者未偵測到、或頭部關鍵點信心不足）為 None，
        # 由呼叫端決定退路。
        # ============================================================
        self.last_tester_head_box: Optional[Tuple[float, float, float, float]] = None
        # 頭部關鍵點（鼻/眼/耳）採用的最低信心度
        self.HEAD_KPT_CONF_TH: float = 0.30
        # 由肩寬推估頭部半徑的係數。頭寬約 0.6 倍肩寬 → 半寬約 0.3；
        # 取 0.40 讓框涵蓋頭髮與下巴，避免只框住五官。
        self.HEAD_SIZE_FROM_SHOULDER: float = 0.40

        # ============================================================
        # 🌟 新增（B1）：前臂與手指的共線性門檻（度）
        # 真正的指向：前臂朝目標伸出，手肘→手腕→食指尖大致共線。
        # 而「抓著袋子／壓在桌面／操作平板」的手，前臂朝下、
        # MediaPipe 偵測到的「食指」卻朝側面，兩者夾角很大。
        # 這正是 2026-09-13 調閱輸出影片後確認的剩餘假陽性類型
        # （49-S7 抓束口袋、90-S8 壓桌面、90-S9 操作平板）。
        #
        # 既有的 align 檢查（is_valid_pointing 內）只比對手掌內部
        # （手腕→食指根 vs 食指根→指尖），完全沒有用到手臂資訊。
        #
        # ⚠️ 2026-09-14 實測：75° 太寬鬆，對結果完全沒有影響
        # （Pointing 的 TP/FP/FN 一格未變，輸出影片上也找不到
        #  `x arm-align` 標記）。90 號 176.9s 的右手壓在布上，
        # 前臂與「食指」都朝左下、夾角本來就小，75° 擋不住。
        # 調整為 45°。真正的指向通常在 0~40° 之間；手在操作物體時
        # 前臂朝下、食指朝側，多半會超過。
        # 驗收方式：輸出影片上應該開始出現 `x arm-align`；
        # 若仍完全沒有，代表 _forearm_vector 回傳 None
        #（手肘/手腕信心低於 ARM_KPT_CONF_TH），要改的是那裡而不是角度。
        # 真陽性紅線：42-S4、49-S7、52-S7 這三個必須存活。
        # 設為 None 可完全停用此檢查。
        # ============================================================
        self.ARM_FINGER_MAX_ANGLE: Optional[float] = 45.0
        # 手肘／手腕關鍵點採用的最低信心度
        self.ARM_KPT_CONF_TH: float = 0.35

    def set_frame_stride(self, stride: int) -> None:
        """🌟 新增（B2）：告知本引擎「每幾幀才被呼叫一次」（= YOLO_SKIP）。

        dwell / hold 這類以呼叫次數累加的門檻會據此換算，
        讓 DWELL_FRAMES、S8_MIN_DWELL_FRAMES、HOLD_FRAMES 維持
        「真實影片幀數」的語意，不隨跳幀設定漂移。
        """
        self.frame_stride = max(1, int(stride))

    def _stride_frames(self, n_frames: int) -> int:
        """把「真實幀數」門檻換算成「呼叫次數」門檻（至少 1 次）。"""
        if self.frame_stride <= 1:
            return n_frames
        return max(1, int(round(n_frames / float(self.frame_stride))))

    def _forearm_vector(self, person: dict, wrist_px: Tuple[int, int]
                        ) -> Optional[np.ndarray]:
        """🌟 新增（B1）：回傳該人「手肘→手腕」的前臂向量。

        左右兩側都算，取 YOLO 手腕與 MediaPipe 手腕較接近的那一側
        （不信任 MediaPipe 的 handedness——第三人稱鏡頭下常被標反，
        這與 calculate_arm_link_score 的處理原則一致）。
        關鍵點信心不足時回傳 None，呼叫端應視為「無法驗證」而不阻擋。
        """
        kpts, confs = person["kpts"], person["confs"]
        best_vec, best_d = None, float("inf")
        for e_idx, w_idx in ((7, 9), (8, 10)):   # (左肘,左腕) / (右肘,右腕)
            if confs[e_idx] <= self.ARM_KPT_CONF_TH or confs[w_idx] <= self.ARM_KPT_CONF_TH:
                continue
            wx, wy = float(kpts[w_idx][0]), float(kpts[w_idx][1])
            d = math.hypot(float(wrist_px[0]) - wx, float(wrist_px[1]) - wy)
            if d < best_d:
                best_d = d
                best_vec = np.array([wx - float(kpts[e_idx][0]),
                                     wy - float(kpts[e_idx][1])], dtype=float)
        if best_vec is None or np.linalg.norm(best_vec) < 1e-6:
            return None
        return best_vec

    def _compute_head_box(self, yolo_people: List[dict], owner: str
                          ) -> Optional[Tuple[float, float, float, float]]:
        """🌟 新增（A1）：用 YOLO-Pose 的鼻/眼/耳關鍵點推出指定角色的頭部框。

        半徑取「關鍵點實際散布」與「由肩寬推估的頭部尺寸」的較大者——
        側臉時只看得到單側耳朵與眼睛，散布會嚴重低估，需要肩寬當地板值。
        回傳 (x1, y1, x2, y2)；無法取得時回傳 None。
        """
        for person in yolo_people:
            if person["owner"] != owner:
                continue
            kpts, confs = person["kpts"], person["confs"]
            pts = [kpts[i] for i in self.FACE_KPT_INDICES
                   if confs[i] > self.HEAD_KPT_CONF_TH]
            if not pts:
                return None
    ...
    def is_valid_pointing(self, landmarks: LegacyHandLms, frame_w: int, frame_h: int, owner: str,
                          frame: Optional[np.ndarray] = None,
                          arm_vec: Optional[np.ndarray] = None) -> Optional[float]:
        ...
        # ============================================================
        # 🌟 新增（B1）：前臂與手指的共線性
        # 真正的指向是「整條手臂朝目標伸出」，手肘→手腕與手腕→食指尖
        # 大致同向；手在操作物體時前臂朝下、食指朝側面，夾角很大。
        # arm_vec 為 None（手肘/手腕關鍵點信心不足）時不阻擋——
        # 無法驗證不等於不合格，維持既有行為避免製造漏偵。
        # ============================================================
        if arm_vec is not None and self.ARM_FINGER_MAX_ANGLE is not None:
            if self.get_angle(arm_vec, idx_tip - wri) > self.ARM_FINGER_MAX_ANGLE:
                return rej("arm-align")
    ...
    def reset_tracking(self):
        ...
        self.last_tester_head_box        = None   # 🌟 新增（A1）：跨影片重置
    ...
    def analyze_interaction(self, frame: np.ndarray, yolo_boxes: List[Tuple[float, float, float, float]], elapsed_ms: Optional[int] = None) -> bool:
        ...
        # 🌟 新增（A1）：每幀先歸零，稍後在人物去重完成後重新計算
        self.last_tester_head_box = None
        ...
        if pose_results and len(pose_results) > 0 and pose_results[0].keypoints is not None:
            ...
            # 🌟 新增（A1）：去重之後才算頭部框，確保取到的是真人而非幽靈偵測
            self.last_tester_head_box = self._compute_head_box(yolo_people, "Tester")
        ...
        if hand_result.hand_landmarks:
            ...
            for hand_idx, hand_landmarks_list in enumerate(hand_result.hand_landmarks):
                ...
                # 🌟 新增（B1）：取出這隻手所屬人物的前臂向量，供共線性檢查
                _arm_vec = None
                for _p in yolo_people:
                    if _p["owner"] == best_owner:
                        _arm_vec = self._forearm_vector(_p, p_wri)
                        break

                idx_angle = self.is_valid_pointing(hand_lms, FRAME_W, FRAME_H, best_owner,
                                                   frame=frame, arm_vec=_arm_vec)
        ...
        for owner in ["Tester", "Child"]:
            if current_frame_pointing[owner]:
                ...
                self.hold_counters[owner] = self._stride_frames(self.HOLD_FRAMES)
            ...
            if self.dwell_counters[owner] >= self._stride_frames(self.DWELL_FRAMES) \
                    and self.hold_counters[owner] > 0:
        ...
        if child_ray_active:
            ...
            dwell_ok   = self.dwell_counters["Child"] >= self._stride_frames(self.S8_MIN_DWELL_FRAMES)
