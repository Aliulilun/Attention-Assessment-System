...
#   0.3 = 各邊外擴 30%，面積約 2.6 倍（目前值）
#   0.8 = 各邊外擴 80%，面積約 6.8 倍（⚠️ 第一版用這個，太大了）
#
...
def process_single_video(video_path, output_dir, model_manager, interaction,
    ...
    # 它的 dwell / hold 是以「被呼叫次數」累加的，而主迴圈只在
    # frame_count % YOLO_SKIP == 0 時呼叫，不換算的話 DWELL_FRAMES=3
    # 在 YOLO_SKIP=2 下就變成 6 個影片幀，同一支影片會跑出不同結果。
        ...
        while cap.isOpened():
            ...
            if active_gaze and active_gaze.get('success'):
                ...
                if current_stage >= 9:
                    ...
                    #
                    # 舊做法是拿 TESTER_ZONE_BBOX（畫面左 35% 的滿版長條）做
                    # 射線相交測試。但兒童的眼睛在畫面右側，視線只要有負的
                    # x 分量，射線必定與這個滿版左欄相交——框測試恆為真，
                    # 真正生效的只有 pitch > -5 且 yaw > 10 兩個角度門檻。
                    # 這正是對照表中 TH 假陽性 122 筆的結構性來源。
                    #
                    # 新做法：用 YOLO-Pose 的鼻/眼/耳關鍵點推出施測者頭部框
                    # （interaction.last_tester_head_box），外擴
                    # TESTER_HEAD_EXPAND 當容差後做相交測試，並套用 A2 的
                    # 射線長度上限。社會性參照在臨床上看的就是「臉」，
                    # 用頭部框才對得上這個構念。
                    #
                    # 退路：頭部框取不到時（施測者未入鏡、關鍵點信心不足）
                    # 退回舊行為，避免整關的 TH 直接消失。
                    # ============================================================
                    ...
                    if _tester_head is not None:
                        ...
                        # 第一版 A1 認為「真正的框測試會涵蓋角度約束」而把
                        # pitch/yaw 門檻拿掉，結果 TH 精確率 84.5% → 77.9%、
                        # 假陽性 13 → 21。實例：90 號 13.9s（Stage 2）
                        # Gaze P=-14.1 Y=-16.6，yaw 是負的、小朋友根本沒看向
                        # 施測者，舊門檻本來擋得掉，拿掉後就誤判成 TH。
