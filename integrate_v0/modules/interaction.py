...
class InteractionEngine:
    def __init__(self, pose_model_path=None, sma_window=5):
        ...
        # 🌟 升級 1：極限下殺信心度，讓 MediaPipe "盡可能" 看到所有手
        # 防線交給後面的幾何演算法，這裡只要確保點一定畫得出來
    ...
    def is_valid_pointing(self, landmarks, frame_w, frame_h, owner):
        ...
        # 如果 MediaPipe 點標錯，把所有手指擠在一起，我們要看破這個幻覺！
        # 正常指向時，食指尖跟中指尖/無名指尖之間一定有一定距離
    ...
    def analyze_interaction(self, frame, yolo_boxes):
        ...
        # 🛡️ 雙手交握封殺名單 (範圍大幅擴大至畫面 12%)
        invalid_hand_indices = set()
        if mp_results.multi_hand_landmarks:
            mp_hands_info = []
            for idx, hand_lms in enumerate(mp_results.multi_hand_landmarks):
                w_x = int(hand_lms.landmark[0].x * FRAME_W)
                w_y = int(hand_lms.landmark[0].y * FRAME_H)
            ...
            for i in range(len(mp_hands_info)):
                for j in range(i + 1, len(mp_hands_info)):
                    ...
                    if dist < FRAME_W * 0.12: # 只要兩隻手在畫面 12% 寬度內，必定是交握或重疊
