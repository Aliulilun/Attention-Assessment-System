...
class ImageNormalizer:
    ...
    核心步驟（與官方一致）：
    1. 用 solvePnP 結果獲得 R_head, t_head（頭部在相機座標系）
    2. 建立正規化座標系：
    ...
    def __init__(self, config: Dict = None):
        ...
        face_model_path = config.get('face_model_path', 'model/gaze/face_model_ethxgaze.txt')
    ...
    def _load_face_model(self, model_path: str) -> np.ndarray:
        ...
        """
        try:
            # 🔥 關鍵修改：加上 encoding='utf-8'，澈底消滅 Windows cp950 解碼亂碼的閃退錯誤！
            face_model = np.loadtxt(model_path, comments='#', encoding='utf-8')
