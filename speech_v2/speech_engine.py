...
PROJECT_NOISE_SAMPLE_RELATIVE_PATH = os.path.join("model", "noise.wav")
DEFAULT_NOISE_TEMPLATE_THRESHOLD = 0.7
DEFAULT_AUTO_NOISE_TEMPLATE_THRESHOLD = 0.7
...
# 【第 22 版：自適應補強模糊對話區段，適用不同長度影片】
MATCHING_ALGORITHM_VERSION = 22
...
class SpeechConfig:
    ...
    dialogue_recovery_enabled: bool = True
    dialogue_recovery_head_sec: float = 35.0
    dialogue_recovery_min_gap_sec: float = 6.0
    dialogue_recovery_padding_sec: float = 0.8
    dialogue_recovery_max_window_sec: float = 35.0
    dialogue_recovery_max_total_sec: float = 90.0
    dialogue_recovery_overlap_sec: float = 0.35
    ...
    noise_template_similarity_min: float = 0.7
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="怪聲參考樣本最低相似度。越高越保守；預設為 0.7",
    )
    parser.add_argument(
        "--noise-max-duration",
        type=float,
...
def resolve_noise_sample_path(
    ...
    default_sample_paths = (
        os.path.join(output_dir, DEFAULT_NOISE_SAMPLE_FILENAME),
        os.path.join(project_dir, PROJECT_NOISE_SAMPLE_RELATIVE_PATH),
    )
    for default_sample_path in default_sample_paths:
        if os.path.isfile(default_sample_path):
...
def build_config(args: argparse.Namespace) -> SpeechConfig:
    ...
    video_path = args.video or os.path.join(project_dir, "video", "8.mp4")
...
def transcribe_with_whisper(config: SpeechConfig) -> Dict[str, Any]:
    ...
    if config.dialogue_recovery_enabled:
        recovery_segments = transcribe_dialogue_recovery(model, result.get("segments", []), config)
        if recovery_segments:
            original_count = len(result.get("segments", []))
            result["segments"] = merge_recovered_segments(
                result.get("segments", []),
                recovery_segments,
                config,
            )
            added_count = len(result["segments"]) - original_count
            if added_count:
                print(f">> 模糊對話補強：新增 {added_count} 個片段")
    return result


def normalize_recovery_windows(
    windows: Sequence[Tuple[float, float]],
    max_total_sec: float,
) -> List[Tuple[float, float]]:
    cleaned = [
    ...
    cleaned.sort(key=lambda window: window[0])
...
def build_dialogue_recovery_windows(
    primary_segments: Sequence[Dict[str, Any]],
    config: SpeechConfig,
) -> List[Tuple[float, float]]:
    windows: List[Tuple[float, float]] = []
    if config.dialogue_recovery_head_sec > 0:
        windows.append((0.0, config.dialogue_recovery_head_sec))

    reliable_segments = []
    for segment in primary_segments:
        text = str(segment.get("text", "")).strip()
        if not text or is_prompt_echo(normalize_text(text)):
            continue
        start_time = float(segment.get("start", 0.0))
        end_time = float(segment.get("end", start_time))
        if end_time <= start_time:
            continue
        reliable_segments.append((start_time, end_time))

    reliable_segments.sort()
    previous_end = 0.0
    for start_time, end_time in reliable_segments:
        gap = start_time - previous_end
        if gap >= config.dialogue_recovery_min_gap_sec:
            padded_start = max(0.0, previous_end - config.dialogue_recovery_padding_sec)
            padded_end = min(
                start_time + config.dialogue_recovery_padding_sec,
                padded_start + config.dialogue_recovery_max_window_sec,
            )
            windows.append((padded_start, padded_end))
        previous_end = max(previous_end, end_time)

    return normalize_recovery_windows(windows, config.dialogue_recovery_max_total_sec)


def transcribe_dialogue_recovery(
    model: Any,
    primary_segments: Sequence[Dict[str, Any]],
    config: SpeechConfig,
) -> List[Dict[str, Any]]:
    recovery_windows = build_dialogue_recovery_windows(primary_segments, config)
    if not recovery_windows:
        return []

    print(
        ">> 模糊對話補強辨識區段："
        + "、".join(f"{start:.1f}s~{end:.1f}s" for start, end in recovery_windows)
    )
    recovered_segments: List[Dict[str, Any]] = []
    for window_start, window_end in recovery_windows:
        temp_path = None
        try:
            ...
            command = [
                ...
                "16000",
                "-af",
                "highpass=f=120,lowpass=f=7800,dynaudnorm=f=150:g=15,volume=2.2",
                "-acodec",
                "pcm_s16le",
                temp_path,
            ]
            ...
            if completed.returncode != 0:
                print(f"⚠️  模糊對話補強音訊建立失敗：{completed.stderr.strip()}")
                continue
            ...
            for segment in result.get("segments", []):
                ...
                segment["end"] = float(segment.get("end", segment["start"])) + window_start
                offset_words = []
                for word in segment.get("words", []) or []:
                    word = dict(word)
                    if "start" in word:
                        word["start"] = float(word["start"]) + window_start
                    if "end" in word:
                        word["end"] = float(word["end"]) + window_start
                    offset_words.append(word)
                segment["words"] = offset_words
                segment["recognition_source"] = "enhanced_dialogue_audio"
...
def segments_overlap_seconds(
    first: Dict[str, Any],
    second: Dict[str, Any],
) -> float:
    start = max(float(first.get("start", 0.0)), float(second.get("start", 0.0)))
    end = min(float(first.get("end", start)), float(second.get("end", start)))
    return max(0.0, end - start)


def merge_recovered_segments(
    original_segments: Sequence[Dict[str, Any]],
    recovery_segments: Sequence[Dict[str, Any]],
    config: SpeechConfig,
) -> List[Dict[str, Any]]:
    merged: List[Dict[str, Any]] = [dict(segment) for segment in original_segments]
    comparable_originals = [
        segment
        for segment in original_segments
        if not is_prompt_echo(normalize_text(str(segment.get("text", ""))))
    ]

    for recovered in recovery_segments:
        start_time = float(recovered.get("start", 0.0))
        end_time = float(recovered.get("end", start_time))
        text = str(recovered.get("text", "")).strip()
        ...
        for original in comparable_originals:
            if segments_overlap_seconds(recovered, original) >= config.dialogue_recovery_overlap_sec:
        ...
        merged.append(dict(recovered))

    merged.sort(key=lambda segment: (float(segment.get("start", 0.0)), float(segment.get("end", 0.0))))
    return merged
        ...
        records.append(
            {
                ...
                "recognition_source": segment.get("recognition_source", "primary"),
                "keywords": [],
                "trigger_events": [],
                "event_type": "speech",
            }
...
def save_cache(
    ...
    payload = {
        ...
        "config": {
            ...
            "dialogue_recovery_enabled": config.dialogue_recovery_enabled,
            "dialogue_recovery_head_sec": config.dialogue_recovery_head_sec,
            "dialogue_recovery_min_gap_sec": config.dialogue_recovery_min_gap_sec,
            "dialogue_recovery_padding_sec": config.dialogue_recovery_padding_sec,
            "dialogue_recovery_max_window_sec": config.dialogue_recovery_max_window_sec,
            "dialogue_recovery_max_total_sec": config.dialogue_recovery_max_total_sec,
            "dialogue_recovery_overlap_sec": config.dialogue_recovery_overlap_sec,
    ...
    with open(config.report_path, "w", encoding="utf-8") as file:
        ...
        if segment_records:
            for record in segment_records:
                ...
                source_label = (
                    "（補強辨識）"
                    if record.get("recognition_source") == "enhanced_dialogue_audio"
                    else ""
                )
                if record["trigger_events"]:
                    file.write(
                        f"⭐ {time_str} {source_label}{record['text']}\n"
                    )
                    ...
                    file.write(f"   {time_str} {source_label}{record['text']}\n")
        else:
