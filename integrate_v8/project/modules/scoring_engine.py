...
class ScoringEngine:
    ...
    def _build_absolute_timeline(self):
        ...
        # ⚠️ C2 待修（2026-09-13 撤回一次失敗的修法，紀錄於此避免重蹈）
        # 問題：e["text"] 是整段 Whisper 轉錄，段落中任何位置出現「畫」字
        #      （例："這張圖畫得很漂亮"）都會被當成 Stage 9 起點，取 min()
        #      只會讓起點被誤判得更早。Stage 11 已於第 288-293 行改用
        #      e["keywords"] 精確比對，Stage 9/10 理應跟進。
        # 為何不能直接照搬：實測 hurry/output/_speech_{42,90,104}/speech_cache.json，
        #      trigger_events 的 keywords 欄位【只會出現「你看」與「看這裡」】，
        #      單字關鍵字「畫」「画」「煙火」「三」從未被 speech_engine 配對成功
        #      （keyword_max_edit_distance=0 + 詞級對齊，單字元關鍵字配不到）。
        #      若改成 keywords-only，t9_all 會是空的 → stage_start_times[9]
        #      永遠不建立 → 【Stage 9 與 Stage 10 會整關消失】，
        #      把 T0 精準度問題惡化成整關漏記。
        # 正確方向：留在 text 比對，但把關鍵字改得更具體
        #      （例：Stage 9 用「畫一幅 / 畫畫 / 畫好了」取代裸的「畫」；
        #        Stage 10 移除裸的「三」與「3」，保留「煙火 / 321 / 三二一」），
        #      或先修正 speech_engine 讓單字關鍵字能被正確配對。
        #      兩者都會移動 T0，需連同 benchmark 重跑一起驗證。
        t9_all = []
        ...
        # ⚠️ C2 待修（同 Stage 9，暫維持 text 比對——原因見上方註解）
        # Stage 10 是這個 bug 最危險的一處：關鍵字清單含裸的「三」與「3」，
        # 對整段轉錄做子字串比對之後再取 min()，幾乎必然抓到影片開頭的語音，
        # 導致整個 Stage 10 判定窗口（T0+10s）落在錯誤的時間點。
        t10_all = []
    ...
    def _update_trigger_records(self, time_sec, current_stage, child_is_pointing_hit,
        ...
        for event in self.speech_events:
            ...
            if current_stage in [1, 2, 3, 4] and ("你看" in event["text"] or "看這裡" in event["text"]):
                ...
                if not already_active:
                    # 🌟 修正（C3）：移除「你看」這個結束訊號。
                    # 「你看」同時是 Stage 1-4 與 Stage 11-14 的起始關鍵字，
                    # 是整段流程中最高頻的詞——第一個出現的「你看」就會把
                    # Stage 9 的判定窗口從 15 秒硬砍到 +3 秒，造成大量漏記。
                    # 只保留語意明確的「畫好了 / 画好了」；找不到就用 T0+15s 的既有退路。
                    s9_end = event["start"] + 15.0
                    for e in self.speech_events:
                        if e["start"] > event["start"] and any(k in e["text"] for k in ["画好了", "畫好了"]):
                            s9_end = e["start"] + 3.0
                            break
                    new_record = create_trigger_record(
        ...
        # Stage 1 前面通常有寒暄，等待秒數（22s）跟代償時間（進入+5.5s，
    ...
    def _apply_frame_to_record(self, record, time_sec, current_stage, child_is_pointing_hit,
        ...
        if record["tb_mode"] is not None and tb_hit and not record["_prev_tb"]:
            ...
            if tb_alternation_ok and time_sec - record["_last_tb_time"] > TB_COOLDOWN_SEC:
                ...
                # ============================================================
                # ⚠️ A7 實驗紀錄（2026-09-14）：此處曾加入「代償 TB 不解開交替鎖」
                #    的條件，已於實測後撤回，請勿重試同樣的做法。
                #
                # 當時假設：Stage 3/4 的 TH 差異率最高（33% / 44%），是因為
                #   EXTREME_TURNING 盲追蹤給出假 TB → 解開交替鎖 → 假 TH。
                #   做法是讓代償來源的 TB 不設 _last_counted_target。
                #
                # 實測結果（10 支 benchmark，對照人工標記）：
                #   淨效果 −2 TP、0 FP 移除，嚴格變差。
                #   影響到的 49-S3 與 104-S3，人工 LI_3 都是 1（確實有看回人）
                #   → 兩個真陽性被改成漏偵。
                #   而這批真正的 Stage 3 假陽性（42、52，人工 LI_3 = 0）
                #   完全沒被碰到，代償路徑根本沒有參與。
                #
                # 結論：代償只在臉部追丟時觸發，而那通常正是「小朋友真的大幅
                #   轉頭看回施測者」的時刻——它命中的是真陽性，不是假陽性。
                #   Stage 3 的假陽性來自別處，最可能是 TESTER_ZONE 判定恆真
                #   （見 docs/程式碼問題清單 A1）。
                # ============================================================
        ...
        if th_allowed:
            if record["th_mode"] == "tester":
                ...
                # 🌟 修正（C1）：robot_boxes 為空時 any([]) 恆為 False。
                # 主迴圈的命中遲滯（GAZE_HIT_HOLD_FRAMES）明明還維持
                # child_is_gazing_at_tester = True，但只要 robot YOLO 掉框那幾幀，
                # TH 就在這一行被靜默吃掉 → Stage 9/10 漏偵（對照表 Stage 9
                # 是唯一 FN 多於 FP 的關卡：2 FP / 3 FN，與此吻合）。
                # 空框時改為直接詢問呼叫端（hurry/main.py:710 與 main.py:595
                # 傳入的 lambda 忽略參數、直接回傳主迴圈已穩定的判定結果）。
                if robot_boxes:
                    th_hit = any(is_gazing_at_box_func(gaze_result, box) for box in robot_boxes)
                else:
                    try:
                        th_hit = bool(is_gazing_at_box_func(gaze_result, None))
                    except Exception:
                        # 呼叫端若傳入真正會用到 box 的函式，退回保守判定
                        th_hit = False
    ...
    def _replay_buffer_into_record(self, record, buffer):
        ...
        for (t, p_hit, gaze_obj, gaze_tester) in buffer:
            self._apply_frame_to_record(
                record, t, record["stage"], p_hit, gaze_obj, gaze_tester,
                None, [],
                # 🌟 修正（C5）：原本硬傳 lambda _g, _b: False。
                # 目前 Stage 1-4 的 th_mode 是 "tester"（走 child_is_gazing_at_tester），
                # 所以還沒出事；但只要未來有 robot_box 階段用到暫存重播，
                # TH 就會被靜默漏記。改為回傳該幀暫存的「看向人/機器人」判定，
                # 與即時路徑語意一致。（_v 預設參數用於捕捉迴圈當下的值）
                (lambda _g, _b, _v=gaze_tester: _v),
            )
    ...
    def _update_gazing_score(self, time_sec, current_stage, child_is_gazing_at):
        ...
        if current_stage in [1, 2, 3, 4, 9]:
            for window_start, window_end in self.you_look_trigger_windows:
                ...
                active_end = 10**9
                if current_stage == 9:
                    # 🌟 修正（C3）：同上，移除「你看」這個過度觸發的結束訊號
                    for e in self.speech_events:
                        if e["start"] > window_start and any(k in e["text"] for k in ["画好了", "畫好了"]):
