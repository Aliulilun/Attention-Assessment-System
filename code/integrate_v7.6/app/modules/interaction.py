...
class InteractionEngine:
    ...
    def __init__(
        ...
        # DWELL_FRAMES=3 實際代表 3 × YOLO_SKIP 幀（註解寫的
        # 「≈0.33s@30fps」只在 YOLO_SKIP=1 時成立）。UI 允許使用者
        # 自由調整跳幀，一旦調成 2，同一支影片就會跑出不同結果。
        ...
        # 這是 2026-09-13 調閱輸出影片後確認的剩餘假陽性類型。
        # 既有的 align 檢查只比對手掌內部，完全沒用到手臂資訊。
        # ⚠️ 2026-09-14 實測：75° 太寬鬆，對結果完全沒有影響
        # （Pointing 的 TP/FP/FN 一格未變，影片上也找不到 `x arm-align`）。
        # 調整為 45°。真正的指向通常在 0~40° 之間。
        # 驗收：影片上應開始出現 `x arm-align`；若仍完全沒有，
        # 代表 _forearm_vector 回傳 None，要改的是那裡而不是角度。
        # 真陽性紅線：42-S4、49-S7、52-S7 必須存活。
        # 設為 None 可完全停用此檢查。
        # ============================================================
