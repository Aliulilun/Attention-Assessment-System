...
# 【第 17 版：整合影片 47 怪聲門檻與 Whisper 幻覺過濾】
MATCHING_ALGORITHM_VERSION = 17
...
def transcribe_with_whisper(config: SpeechConfig) -> Dict[str, Any]:
    ...
    # 🌟 裝置選擇與顯示記憶體不足時的退路
    #
    # large-v3 有 15.5 億參數，這裡以 fp16=False（FP32）執行，
    # 光權重就 6.2 GB，加上推論的中間結果，實際約需 8~10 GB VRAM。
    #
    # 原本只判斷 torch.cuda.is_available()：顯卡存在但記憶體不足時，
    # 會在載入模型或辨識途中丟 CUDA out of memory。這個例外會一路
    # 傳到呼叫端被 try/except 接住，結果是「語音辨識靜默失效」——
    # 沒有觸發時間窗、Stage 8/9/10 全部失準，畫面上卻看不出異常。
    # 對於要發佈給其他機器使用的版本，這是最危險的失敗模式。
    #
    # 改為：CUDA OOM 時清掉快取並改用 CPU 重跑。CPU 慢很多
    # （一支 6 分鐘影片約 10~20 分鐘），但結果正確，
    # 而且訊息會明白告訴使用者發生了什麼事。
    # ============================================================
    def _run(device):
        print(f">> 載入 Whisper 模型：{config.model_name}（裝置：{device}）")
    ...
    def _is_oom(exc):
        ...
        torch 的 OOM 依版本可能是 torch.cuda.OutOfMemoryError 或普通的
        RuntimeError，訊息也可能來自 cuBLAS / cuDNN 的配置失敗，
        所以型別與訊息兩邊都比對。
        """
        ...
        if "cublas_status_alloc_failed" in msg or "cudnn_status_alloc_failed" in msg:
            return True
        return "cuda error" in msg and "memory" in msg

    if not torch.cuda.is_available():
        print(">> ⚠️  未偵測到 CUDA，Whisper 將使用 CPU（速度顯著較慢）")
        return _run("cpu")

    try:
        _vram = torch.cuda.get_device_properties(0).total_memory / 1024 ** 3
        print(f">> GPU 顯示記憶體：{_vram:.1f} GB"
              f"（{config.model_name} 以 FP32 執行約需 8~10 GB）")
    except Exception:
        pass
    ...
    try:
        ...
        if not _is_oom(exc):
            raise
        print("=" * 60)
        print("⚠️  顯示記憶體不足，Whisper 無法在 GPU 上執行。")
        print(f"    原始錯誤：{exc}")
        print("    改用 CPU 重新辨識——結果一樣正確，但會慢很多")
        print("    （一支 6 分鐘的影片大約需要 10~20 分鐘）。")
        print("    辨識結果會寫進快取，同一支影片之後不會再重跑。")
        print("=" * 60)
        import gc
        gc.collect()
        try:
