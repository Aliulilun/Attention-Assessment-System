...
def parse_gaze_log(file_path):
    ...
    for stage, cols in stages_schema.items():
        ...
        # 🌟 新增：文字檔中完全沒有 Stage 7 區塊（該階段未被偵測到）
        # → 整組欄位標示「沒測到」，與「有測到但無反應（x / F）」明確區分
        if stage == 7 and not match:
            for field in ['T0', 'TB', 'TH', 'Pointing次數', 'TB次數', 'TH次數', 'total分數', '反應等級']:
                row_data[f'Stage7_{field}'] = '沒測到'
            continue
