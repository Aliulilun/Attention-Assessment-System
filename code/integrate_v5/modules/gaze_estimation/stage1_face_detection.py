...
class FaceDetector:
    def __init__(self, config: Dict = None):
        ...
        # 🌟 硬體優化邏輯：先偵測 CUDA，供 YOLO 與 MediaPipe delegate 共用
        self.use_cuda = torch.cuda.is_available()
        if self.use_cuda:
            print(f">>> [FaceDetector] 偵測到 GPU：{torch.cuda.get_device_name(0)}，啟用 CUDA 加速！")
        else:
            print(">>> [FaceDetector] ⚠️ 沒有偵測到 CUDA，將使用 CPU 進行推論，速度會較慢。")

        # 1. 初始化 YOLO 模型
        yolo_path = config.get('yolo_model_path', 'model/gaze/nano.pt')
        ...
        mp_task_path = config.get('face_landmarker_task', 'model/gaze/face_landmarker.task')
        if not Path(mp_task_path).exists():
            raise FileNotFoundError(f"找不到 MediaPipe 模型檔案: {mp_task_path}")

        def _build_face_options(delegate):
            return FaceLandmarkerOptions(
                base_options=BaseOptions(model_asset_path=mp_task_path, delegate=delegate),
        ...
        # 🌟 先檢查 CUDA 再決定 delegate；GPU delegate 建構失敗（例如 mediapipe
        #    Windows 版 build 未啟用 GPU）時自動退回 CPU delegate，避免直接崩潰
        if self.use_cuda:
            try:
                self.face_landmarker = FaceLandmarker.create_from_options(
                    _build_face_options(BaseOptions.Delegate.GPU)
                )
            except Exception as _gpu_err:
                print(f">>> [FaceDetector] ⚠️ MediaPipe GPU delegate 不可用（{_gpu_err}），退回 CPU")
                self.face_landmarker = FaceLandmarker.create_from_options(
                    _build_face_options(BaseOptions.Delegate.CPU)
                )
        else:
            self.face_landmarker = FaceLandmarker.create_from_options(
                _build_face_options(BaseOptions.Delegate.CPU)
    ...
    def detect(self, image: np.ndarray) -> Optional[Dict]:
        ...
        results = self.yolo_model.predict(
            focused_image, conf=self.yolo_conf, imgsz=640, verbose=False,
            device=0 if self.use_cuda else "cpu",
            quantize=16 if self.use_cuda else None
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"\n 錯誤: {e}")
        print("請檢查 model/gaze/ 目錄下是否具備 nano.pt 與 face_landmarker.task")
