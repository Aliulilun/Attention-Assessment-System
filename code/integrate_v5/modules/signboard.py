...
class SignboardTracker:
    ...
    def __init__(self, allowlist='12345678'):
        ...
        # 🌟 硬體優化邏輯：先偵測 CUDA，再決定是否啟用 EasyOCR 的 GPU 推論
        self.use_cuda = torch.cuda.is_available()
        if self.use_cuda:
            print(f">>> [SignboardTracker] 偵測到 GPU：{torch.cuda.get_device_name(0)}，啟用 CUDA 加速！")
        else:
            print(">>> [SignboardTracker] ⚠️ 沒有偵測到 CUDA，將使用 CPU 進行推論，速度會較慢。")
