...
- **換牌位置凍結 (`_occlusion_freeze`)**：偵測到手部遮擋牌子（膚色像素比 > 閾值）時，KCF 追蹤器繼續內部運行但結果不寫入，掃描框釘在原位；手退走後 OCR 在原位找到新牌即自動解凍
- 批次模式使用 `initialize_roi_auto(first_frame)`（自動框選右下 1/4）；單片模式使用 `initialize_roi(first_frame)`（互動框選）
...
- **身分識別防禦**：手臂關節連動分數 (Arm Link Score) 區分施測者與兒童
...
```bash
pip install ultralytics mediapipe openai-whisper easyocr opencv-python scipy numpy pyyaml
```
